...
def generate_pdf(request, pk):
    ...
    # Add bullets to work description
    if invoice.work_description:
        lines = [line.strip() for line in invoice.work_description.split('\n') if line.strip()]
...
def analytics_view(request):
    ...
    total_revenue = all_invoices.aggregate(revenue=Sum(F('amount') * F('quantity')))['revenue'] or 0
    ...
    for i in range(5, -1, -1):
        ...
        from django.db.models import F
        m_rev = Invoice.objects.filter(date__gte=m_date, date__lt=n_month).aggregate(revenue=Sum(F('amount') * F('quantity')))['revenue'] or 0
        last_6_months_data.append({'month': m_name, 'revenue': float(m_rev)})

    # Top clients from the filtered queryset
    from django.db.models import F
    top_clients = all_invoices.values('client_name').annotate(total=Sum(F('amount') * F('quantity')), count=Count('id')).order_by('-total')[:5]
...
def export_analytics_csv(request):
    ...
    for invoice in all_invoices:
        unit_amt = float(invoice.amount)
        qty = float(invoice.quantity)
        subtotal = unit_amt * qty
        vat = subtotal * 0.05
        writer.writerow([
